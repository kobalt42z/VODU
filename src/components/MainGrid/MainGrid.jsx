          ...
          gap-y-5 grid-flow-row md:p-5">
            ...
            {props.episodes && props.episodes.map((episode) => {
                return  <Link to={`/tv/${episode.show_id}/seasons/${episode.season_number}/episode/${episode.episode_number}/?tvn=${props.tvName}`} >
                    ...
                    <MovieCard key={episode.id} grow imgUrl={episode.still_path?POSTER_BASE_URL+episode.still_path : 'https://htgindustry.com/wp-content/uploads/improve-video-play-rate.png'} title={episode.episode_number+' : ' +episode.name} alt={episode.name} />
                    </div>
                    </Link>
            })}
